...
class RunCatalog:
    ...
    def batch_units(
        ...
        *,
        ...
        """Pack *units* within *max_gb* and *max_units*; oversized units stand alone."""

        if max_gb is not None and max_gb <= 0:
            raise ValueError("max_gb must be positive")
        if max_units is not None and max_units <= 0:
            raise ValueError("max_units must be positive")
        materialized = sorted(units, key=lambda item: (-item.total_size_gb, item.unit_id))
        batches: list[list[ProcessingUnit]] = []
        sizes: list[float] = []
        for unit in materialized:
            placed = False
            for index, batch in enumerate(batches):
                size_ok = max_gb is None or sizes[index] + unit.total_size_gb <= max_gb
                count_ok = max_units is None or len(batch) < max_units
                if size_ok and count_ok:
                    batch.append(unit)
                    sizes[index] += unit.total_size_gb
                    placed = True
                    break
            if not placed:
                batches.append([unit])
                sizes.append(unit.total_size_gb)
        return batches
