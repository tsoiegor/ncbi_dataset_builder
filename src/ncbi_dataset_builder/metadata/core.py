...
class EntrezClient:
    ...
    def resolve_uids(
        ...
        *,
        ...
        Requests contain at most *request_chunk_size* accessions, and *refresh* bypasses
        ...
        queries = [
            " OR ".join(
                f'"{accession}"[{field}]'
                for accession in accessions[start : start + request_chunk_size]
            )
            for start in range(0, len(accessions), request_chunk_size)
        ]
        ...
        for query in reporter.track(queries, f"Resolve {database} accessions", unit="chunks"):
...
class SraClient:
    ...
    def fetch_runinfo_ids(
        self, ids: list[str], *, request_chunk_size: int = 200, refresh: bool = False
    ) -> RunCatalog:
        """Fetch RunInfo for SRA *ids* in chunks of *request_chunk_size*.
        ...
        *refresh* bypasses cached Entrez responses.
        ...
        uids = self.entrez.resolve_uids(
            "sra",
            ids,
            request_chunk_size=min(request_chunk_size, 50),
            refresh=refresh,
        )
        records: list[dict[str, Any]] = []
        starts = range(0, len(uids), request_chunk_size)
        for start in _client_progress(self.entrez).track(
            starts,
            "Fetch SRA RunInfo by IDs",
            total=(len(uids) + request_chunk_size - 1) // request_chunk_size,
            unit="chunks",
        ):
            payload = self.entrez.get(
                "efetch.fcgi",
                {
                    "db": "sra",
                    "id": ",".join(uids[start : start + request_chunk_size]),
                    "rettype": "runinfo",
                    "retmode": "text",
                },
                refresh=refresh,
    ...
    def fetch_packages(
        ...
        *,
        ...
        """Fetch package XML for SRA *accessions* in chunks of *request_chunk_size*.
        ...
        *include_raw* retains complete XML trees alongside normalized records,
        ...
        parameters = [
            ...
            for start in range(0, len(uids), request_chunk_size)
        ]
        ...
        for params in reporter.track(parameters, "Fetch and parse SRA package XML", unit="chunks"):
            payload = self.entrez.get(
                "efetch.fcgi",
                params,
                refresh=refresh,
...
class BioSampleClient:
    ...
    def fetch(
        ...
        *,
        ...
        """Fetch BioSample *accessions* in chunks of *request_chunk_size*.
        ...
        *include_raw* retains XML trees, and *refresh* bypasses cached Entrez
        ...
        for params in reporter.track(parameters, "Fetch and parse BioSample XML", unit="chunks"):
            payload = self.entrez.get(
                "efetch.fcgi",
                params,
                refresh=refresh,
