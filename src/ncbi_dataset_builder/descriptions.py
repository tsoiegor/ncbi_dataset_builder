"""Compact biological descriptions, with explicit legacy-value preservation.
...
# The original HTML parser dropped the first character of some attribute names.
# Repair the spelling, but preserve its stored value.
LEGACY_KEY_ALIASES = {
    "EO Accession": "GEO Accession",
    "NA-STATUS-ID": "ENA-STATUS-ID",
    "GDC_submission_date": "NGDC_submission_date",
}
    ...
    *,
    ...
    for sample in reporter.track(bundle.sra_samples, "Build training descriptions", unit="samples"):
        ...
        if legacy.get("ID", accession) != accession:
            raise ValueError(f"Legacy ID does not match SRA Sample {accession}")
        output: dict[str, Any] = {"ID": accession}
        # Historical values are the compatibility baseline, including custom fields.
        for key, value in legacy.items():
            if key == "ID":
                continue
            _add(output, LEGACY_KEY_ALIASES.get(key, key), value, keep_missing=True)
        ...
        # Remove only new, redundant source labels. Never remove a legacy value.
...
def missing_legacy_values(legacy: dict[str, Any], compact: dict[str, Any]) -> list[str]:
    ...
    def leaves(value):
        ...
        return [_comparison(sanitize_presentation_markup(value))]

    available = set(leaves(compact))
    return [key for key, value in legacy.items() if not set(leaves(value)) <= available]
